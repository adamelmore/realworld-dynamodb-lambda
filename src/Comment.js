...
const commentsTable = process.env.COMMENTS_TABLE_NAME
...
module.exports = {
  ...
  async get(event) {
    ...
    const comments = (
      await Util.DocumentClient.query({
        TableName: commentsTable,
        IndexName: 'article',
    ...
    for (let i = 0; i < comments.length; ++i) {
      comments[i].author = await User.getProfileByUsername(comments[i].author, authenticatedUser)
    }
  ...
  async delete(event) {
    ...
    if (comment.author !== authenticatedUser.username) {
      return Util.envelop(`Only comment author can delete: [${comment.author}]`, 422)
    }
