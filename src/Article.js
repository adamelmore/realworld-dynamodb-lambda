...
module.exports = {
  ...
  async create(event) {
    ...
    const slug =
      slugify(articleData.title) + '-' + ((Math.random() * Math.pow(36, 6)) | 0).toString(36)
    const article = {
      slug,
  ...
  async update(event) {
    ...
    if (!articleMutation.title && !articleMutation.description && !articleMutation.body) {
      return Util.envelop(
    ...
    if (article.author !== authenticatedUser.username) {
      return Util.envelop('Article can only be updated by author: ' + `[${article.author}]`, 422)
    }
  ...
  async delete(event) {
    ...
    // Ensure article is authored by authenticatedUser
    if (article.author !== authenticatedUser.username) {
      return Util.envelop('Article can only be deleted by author: ' + `[${article.author}]`, 422)
    }
  ...
  async favorite(event) {
    ...
    if (shouldFavorite) {
      ...
      article.favoritedBy = article.favoritedBy.filter((e) => e !== authenticatedUser.username)
      /* istanbul ignore next */
  ...
  async list(event) {
    ...
    if (
      (params.tag && params.author) ||
      (params.author && params.favorited) ||
      (params.favorited && params.tag)
    ) {
      return Util.envelop('Only one of these can be specified: [tag, author, favorited]', 422)
    }
    const queryParams = {
      TableName: articlesTable,
      IndexName: 'updatedAt',
 ...
 */
async function queryEnoughArticles(queryParams, authenticatedUser, limit, offset) {
  ...
  while (queryResultItems.length < offset + limit) {
    const queryResult = await Util.DocumentClient.query(queryParams).promise()
  ...
  queryResultItems
    ...
    .forEach((a) => articlePromises.push(transformRetrievedArticle(a, authenticatedUser)))
...
async function transformRetrievedArticle(article, authenticatedUser) {
  ...
  if (article.favoritedBy && authenticatedUser) {
    article.favorited = article.favoritedBy.includes(authenticatedUser.username)
    delete article.favoritedBy
  }
  article.author = await User.getProfileByUsername(article.author, authenticatedUser)
  return article
}
