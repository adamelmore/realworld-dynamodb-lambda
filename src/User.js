...
module.exports = {
  ...
  async login(event) {
    ...
    if (!bcrypt.compareSync(user.password, userWithThisEmail.Items[0].password)) {
  ...
  async follow(event) {
    ...
    if (shouldFollow) {
      if (user.followers && !user.followers.values.includes(authenticatedUser.username)) {
        user.followers.values.push(authenticatedUser.username)
      } else {
        user.followers = Util.DocumentClient.createSet([authenticatedUser.username])
      }
    } else {
      if (user.followers && user.followers.values.includes(authenticatedUser.username)) {
        user.followers.values = user.followers.values.filter((e) => e != authenticatedUser.username)
    ...
    if (shouldFollow) {
      if (authenticatedUser.following && !authenticatedUser.following.values.includes(username)) {
...
function mintToken(aUsername) {
  return jwt.sign({username: aUsername}, Util.tokenSecret, {expiresIn: '2 days'})
}
...
async function getProfileByUsername(aUsername, aAuthenticatedUser) {
  ...
  if (user.followers && aAuthenticatedUser) {
    profile.following = user.followers.values.includes(aAuthenticatedUser.username)
  }
  return profile
}
