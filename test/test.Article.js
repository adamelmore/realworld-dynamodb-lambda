...
describe('Article', async () => {
  before(async () => {
    globals.authorUser = await TestUtil.createTestUser(`author-${TestUtil.randomString()}`)

    globals.authoressUser = await TestUtil.createTestUser(`authoress-${TestUtil.randomString()}`)

    globals.nonAuthorUser = await TestUtil.createTestUser(`non-author-${TestUtil.randomString()}`)
  })
  ...
  describe('Get', async () => {
    it('should get article by slug', async () => {
      const retrievedArticle = (
        await axios.get(`/articles/${globals.createdArticleWithoutTags.slug}`)
      ).data.article
    ...
    it('should get article with tags by slug', async () => {
      const retrievedArticle = (await axios.get(`/articles/${globals.createdArticleWithTags.slug}`))
        .data.article
    ...
    it('should disallow unknown slug', async () => {
      await axios.get(`/articles/${Math.random().toString(36).substring(7)}`).catch((res) => {
  ...
  describe('Update', async () => {
    ...
    it('should disallow missing mutation', async () => {
      await axios.put(`/articles/${globals.createdArticleWithTags.slug}`, {}).catch((res) => {
    ...
    it('should disallow empty mutation', async () => {
      await axios
        .put(`/articles/${globals.createdArticleWithTags.slug}`, {
          article: {},
        })
        .catch((res) => {
    ...
    it('should disallow updating non-existent article', async () => {
      await axios
        .put(
          `/articles/foo-${globals.createdArticleWithTags.slug}`,
          {
            article: {title: 'newtitle'},
          },
          {
  ...
  describe('Favorite', async () => {
    it('should favorite article', async () => {
      const favoritedArticle = (
        await axios.post(
          `/articles/` + `${globals.createdArticleWithoutTags.slug}/favorite`,
          {},
          {
      ...
      assert(favoritedArticle.favorited === true, `Expected article to have been favorited`)
      ...
      assert(retrievedArticle.favorited === true, `Expected article to have been favorited`)
    })
    ...
    it('should disallow favoriting unknown article', async () => {
      await axios
        .post(
          `/articles/` + `${globals.createdArticleWithoutTags.slug}_foo/favorite`,
          {},
          {
            headers: {Authorization: `Token ${globals.nonAuthorUser.token}`},
          }
    ...
    it('should unfavorite article', async () => {
      const unfavoritedArticle = (
        await axios.delete(`/articles/` + `${globals.createdArticleWithoutTags.slug}/favorite`, {
          headers: {Authorization: `Token ${globals.nonAuthorUser.token}`},
        })
      ).data.article
      assert(unfavoritedArticle.favorited === false, `Expected article to have been unfavorited`)
    })
  })

  describe('Delete', async () => {
    it('should delete article', async () => {
      await axios.delete(`/articles/${globals.createdArticleWithoutTags.slug}`, {
        headers: {Authorization: `Token ${globals.authorUser.token}`},
      })

      // Assert article is deleted
      await axios
        .get(`/articles/${globals.createdArticleWithoutTags.slug}`)
        .catch((res) => TestUtil.assertError(res, /Article not found/))
    })
    ...
    it('should disallow deleting article by non-author', async () => {
      await axios
        .delete(`/articles/${globals.createdArticleWithTags.slug}`, {
          headers: {Authorization: `Token ${globals.nonAuthorUser.token}`},
        })
        .catch((res) => TestUtil.assertError(res, /Article can only be deleted by author/))
    })
  })
  ...
  describe('List', async () => {
          ...
          (
            await axios.post(
              ...
              },
              {
                headers: {
                  Authorization:
                    'Token ' + (i % 2 ? globals.authorUser.token : globals.authoressUser.token),
                },
              }
            )
          ).data.article
    ...
    it('should list articles', async () => {
      const allArticles = (await axios.get(`/articles`)).data.articles
      // TODO: Assert on retrieved articles
      allArticles
    })

    it('should list articles with tag', async () => {
      const articles_tag_7 = (await axios.get(`/articles?tag=tag_7`)).data.articles
      articles_tag_7

      const articles_tag_mod_3_2 = (await axios.get(`/articles?tag=tag_mod_3_2`)).data.articles
    ...
    it('should list articles by author', async () => {
      const articles_of_authoress = (
        await axios.get(`/articles?author=${globals.authoressUser.username}`)
      ).data.articles
    ...
    it('should list articles favorited by user', async () => {
      const favorited_articles = (
        await axios.get(`/articles?favorited=${globals.nonAuthorUser.username}`)
      ).data.articles
      favorited_articles
    ...
    it('should list articles by limit/offset', async () => {
      const articles_batch_1 = (
        await axios.get(`/articles?author=${globals.authorUser.username}&limit=2`)
      ).data.articles
      articles_batch_1

      const articles_batch_2 = (
        await axios.get(`/articles?author=${globals.authorUser.username}` + `&limit=2&offset=2`)
      ).data.articles
      articles_batch_2
