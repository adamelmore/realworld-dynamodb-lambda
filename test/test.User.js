...
describe('User', async () => {
  describe('Create', async () => {
    it('should create user', async () => {
      const createdUser = (await axios.post(`/users`, {user: userToCreate})).data.user
      assertUserEquals(createdUser, userToCreate)
    })

    it('should disallow same username', async () => {
      await axios.post(`/users`, {user: userToCreate}).catch((res) => {
    ...
    it('should enforce required fields', async () => {
      await axios.post(`/users`, {}).catch((res) => {
  ...
  describe('Get', async () => {
    ...
    it('should disallow bad tokens', async () => {
      await axios
        .get(`/user`)
        .catch((res) => TestUtil.assertError(res, /Token not present or invalid/))
      await axios
        .get(`/user`, {headers: {foo: 'bar'}})
        .catch((res) => TestUtil.assertError(res, /Token not present or invalid/))
      await axios
        .get(`/user`, {headers: {Authorization: 'foo'}})
        .catch((res) => TestUtil.assertError(res, /Token not present or invalid/))
      await axios
        .get(`/user`, {
  ...
  describe('Profile', async () => {
    it('should get profile', async () => {
      const profile = (await axios.get(`/profiles/${userToCreate.username}`)).data.profile
      // TODO: Assert on profile
      profile
    })

    it('should disallow unknown username', async () => {
      await axios.get(`/profiles/foo_${Math.random().toString(36)}`).catch((res) => {
        TestUtil.assertError(res, /User not found/)
      })
    })
    ...
    it('should follow/unfollow user', async () => {
      ...
      const retrievedFollowedProfile = (
        await axios({
          method: 'GET',
      ...
      const retrievedFollowedProfileNoAuth = (
        await axios({
          method: 'GET',
          url: `/profiles/followed_user`,
        })
  ...
  describe('Update', async () => {
    ...
    it('should disallow reusing email', async () => {
      const newUser = await TestUtil.createTestUser(`user2-${Math.random().toString(36)}`)
      await axios
        .put(
          `/user`,
          {
