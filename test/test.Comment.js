...
describe('Comment', async () => {
  before(async () => {
    ...
    globals.commenterUser = await TestUtil.createTestUser(`commenter-${TestUtil.randomString()}`)
    globals.testArticle = (
      await axios.post(
        `/articles`,
        {
  ...
  describe('Get', async () => {
    it('should get all comments for article', async () => {
      const retrievedComments = (await axios.get(`/articles/${globals.testArticle.slug}/comments`))
        .data.comments
  ...
  describe('Delete', async () => {
    it('should delete comment', async () => {
      await axios.delete(
        `/articles/${globals.testArticle.slug}` + `/comments/${globals.createdComments[0].id}`,
        {
          headers: {Authorization: `Token ${globals.commenterUser.token}`},
        }
      )
    ...
    it('only comment author should be able to delete comment', async () => {
      await axios
        .delete(
          `/articles/${globals.testArticle.slug}` + `/comments/${globals.createdComments[1].id}`,
          {
            headers: {Authorization: `Token ${globals.authorUser.token}`},
          }
        )
    ...
    it('should disallow deleting unknown comment', async () => {
      await axios
        .delete(`/articles/${globals.testArticle.slug}` + '/comments/foobar_id', {
          headers: {Authorization: `Token ${globals.authorUser.token}`},
        })
        .catch((res) => {
